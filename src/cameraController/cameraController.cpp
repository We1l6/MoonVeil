...
void CameraController::Update(float deltaTime, const Vector2& targetPosition) {
    ...
    camera.target.x = Lerp(camera.target.x, targetPosition.x + 70, smoothness * deltaTime);
    camera.target.y = Lerp(camera.target.y, targetPosition.y + 64, smoothness * deltaTime);
}