...
Player::Player(TileMap& tilemap, ObjectAttributes objectAttributes, float hitPoints, std::vector<std::shared_ptr<Ability>>& gameObjects)
...
void Player::HandleInput(float deltaTime) {
    ...
    if (IsKeyDown(KEY_W)) newPosition.y -= m_objectAttributes.velocity.y * deltaTime;
    if (IsKeyDown(KEY_S)) newPosition.y += m_objectAttributes.velocity.y * deltaTime;

    if (IsKeyPressed(KEY_SPACE)) TakeDamage(10);
