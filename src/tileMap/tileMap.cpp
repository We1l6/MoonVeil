...
void TileMap::LoadTextures()
{
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            Texture2D texture =
                ResourceManager::GetSubTexture("resources/snow.png", i, j);
            if (texture.id == 0)
            {
                TraceLog(LOG_ERROR,
...
void TileMap::Draw()
{
    for (int y = 0; y < mapHeight; y++)
    {
        for (int x = 0; x < mapWidth; x++)
        {
            ...
            {
            case '#':
                DrawTexture(tex_wall, pos.x, pos.y, WHITE);
                break;
            case 'w':
                DrawTexture(tex_water, pos.x, pos.y, WHITE);
                break;
            case 'y':
                DrawTexture(tex_yellow, pos.x, pos.y, WHITE);
                break;
            case 'p':
                DrawTexture(tex_purple, pos.x, pos.y, WHITE);
                break;
            case 's':
                DrawTexture(tex_snow[0], pos.x, pos.y, WHITE);
                break;
            case '1':
                DrawTexture(tex_snow[1], pos.x, pos.y, WHITE);
                break;
            case '2':
                DrawTexture(tex_snow[2], pos.x, pos.y, WHITE);
                break;
            case '3':
                DrawTexture(tex_snow[3], pos.x, pos.y, WHITE);
                break;
            case '4':
                DrawTexture(tex_snow[4], pos.x, pos.y, WHITE);
                break;
            case '5':
                DrawTexture(tex_snow[5], pos.x, pos.y, WHITE);
                break;
            case '6':
                DrawTexture(tex_snow[6], pos.x, pos.y, WHITE);
                break;
            case '7':
                DrawTexture(tex_snow[7], pos.x, pos.y, WHITE);
                break;
            case '8':
                DrawTexture(tex_snow[8], pos.x, pos.y, WHITE);
                break;
            default:
                DrawTexture(tex_grass, pos.x, pos.y, WHITE);
                break;
            }
        }
    }
