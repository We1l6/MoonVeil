...
Ability::Ability(AbilityAttribute abilityAttribute, ObjectAttributes objectAttributes)
    : GameObject(objectAttributes), m_abilityAttribute(abilityAttribute){

}

void Ability::Activate() {
    if (m_abilityAttribute.currentCooldown <= 0.0f) {
        std::cout << "Ability " << m_abilityAttribute.name << " activated!" << std::endl;
        m_abilityAttribute.currentCooldown = m_abilityAttribute.cooldown;
    } else {
        std::cout << "Ability " << m_abilityAttribute.name << " is on cooldown!" << std::endl;
    }
}
